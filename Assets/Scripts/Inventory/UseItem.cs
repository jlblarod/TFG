...
public class UseItem : MonoBehaviour
{
    public void ApplyItemEffect(ItemSO item)
    {
        ...
        if (item.currentHealth > 0 && playerHealth != null) playerHealth.changeHealth(item.currentHealth);
        ...
        if (item.duration > 0) StartCoroutine(EffectTimer(item, item.duration));
    }
